...
-   R01CA222847 ([A. Snook](https://orcid.org/0000-0002-9104-4505), [T. Zhan](https://orcid.org/0000-0001-9971-4844))
...
```{r}
d0 = moodie2ELISpot(`^rep[1-5]$` ~ Antigen | Week + Subject, data = snook, control = 'DMSO')
```
...
```{r}
#m1 = mclapply(ds, FUN = maxT_santos, null.value = 1)
#m2 = mclapply(ds, FUN = maxT_santos, null.value = 2)
#m = mapply(FUN = c, 'null1' = m1, 'null2' = m2, SIMPLIFY = FALSE)
```


```{r}
Wk5.Ad5 = maxT_santos_test(data1 = ds$Wk5.Ad5, data0 = ds$Wk1.Ad5)
```


@fig-XX presents an example visualization of Westfall-Yang max-$T$ correction
for a total of 46 two-sided difference-of-difference hypotheses. 
The nebula of points represents the successive maxima (\ref{successM2}).
The bigger point per hypothesis is the test statistic (\ref{santosT2}).
The labels on the left vertical axis are permutation adjusted $p$-values (\ref{perm_p}) and monotonicity constraints enforced (\ref{perm_p_mono}).
The labels on the right vertical axis are the values of the test statistic (\ref{santosT2}).
Hypothesis with monotonicity constraints enforced $\tilde{p}>.05$ are colored red.
The line connecting all hypotheses does not have a statistical meaning, 
but only to let all test statistics (\ref{santosT2}) stand out to the naked eye.
...
Let $\mu_1$ and $\mu_0$ be the population means of treatment and control, respectively.
To test the hypothesis $H_0: \mu_1 - c\cdot\mu_0 = 0$, let $(\bar{x}_1, s^2_1, n_1)$ and $(\bar{x}_0, s^2_0, n_0)$ be the sample mean, sample standard deviation and sample size of treatment and control, respectively, and consider a distribution free test statistic

$$
t = \dfrac{\bar{x}_1 - c\cdot\bar{x}_0}{s_{\bar\Delta}}
$$ {#eq-santosT}

The standard deviation 
$s^2_{\bar\Delta}= s^2_1/n_1 + c^2s^2_0/n_0$
has degree of freedom [@Welch47; @Satterthwaite46]
$$
...
We apply Westfall-Young max-$T$ correction [@Dudoit03; @WestfallYoung93] to multiple hypotheses $H_j$, $j=1,\cdots,m$. Let $t_j$ be the distribution free test statistic (@eq-santosT) of hypothesis $H_j$ in the original data, $j=1,\cdots,m$. Let $r_1,\cdots,r_m$ be the decreasing order statistics, such that 
...
Let $t_{j,b}$ be the distribution free test statistic (@eq-santosT) of hypothesis $H_j$ in the $b$-th permutation of the treatment and control subjects, $j=1,\cdots,m$, $b=1,\cdots,B$.
...
Suppose we are interested in comparing the treatment effect at two time points $\phi$ versus $\psi$,
i.e., the *time* difference of *treatment* difference.
...
$$
H_0: (\mu_{1,\phi} - c_\phi\cdot\mu_{0,\phi}) - (\mu_{1,\psi} - c_\psi\cdot\mu_{0,\psi}) = 0
$$ {#eq-hypothesis2}
...
The standard deviation 
$s^2_{\bar\Delta,\phi}=s^2_{1,\phi}/n_{1,\phi} + c_\phi^2s^2_{0,\phi}/n_{0,\phi}$
...
Similarly, at time point $\psi$, we have distribution free test statistic $t_\psi$,
standard deviation $s^2_{\bar\Delta,\psi}$ and degree of freedom $\text{df}_\psi$.
To test the difference-of-difference hypothesis (@eq-hypothesis2), we have a distribution free difference-of-difference test statistic
