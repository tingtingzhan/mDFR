...
```{r}
st1 |>
  lapply(FUN = free_t, null.value = 1) |>
  lapply(FUN = maxT) |>
  lapply(FUN = as.data.frame.maxT) |>
  do.call(what = rbind.data.frame, args = _) |>
  reactable::reactable(
...
```{r}
st1 |>
  lapply(FUN = free_t, null.value = 2) |>
  lapply(FUN = maxT) |>
  lapply(FUN = as.data.frame.maxT) |>
  do.call(what = rbind.data.frame, args = _) |>
  reactable::reactable(
...
We apply Westfall-Young max-$T$ correction [@Dudoit03; @WestfallYoung93] to multiple hypotheses $H_j$, $j=1,\cdots,m$. Let $t_j$ be the distribution free test statistic (@eq-freeT) of hypothesis $H_j$ in the original data, $j=1,\cdots,m$. Let $r_1,\cdots,r_m$ be the decreasing order statistics, such that
