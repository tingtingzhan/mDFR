...
Function `santosT()` returns a `numeric` `vector` of $\bar{x}_1 - c\bar{x}_0$, i.e., the numerator of distribution free test statistic (@eq-santosT), with `attributes`,

-   `attr(.,'df')`, degree of freedom (@eq-santosT_df)
-   `attr(.,'stderr')`, standard error $s_{\bar\Delta}$, i.e., the denominator of distribution free test statistic (@eq-santosT)
-   `attr(.,'stderr2')`, standard deviation $s^2_{\bar\Delta}$, included due to the extreme time-sensitivity of function `santosT2()`
